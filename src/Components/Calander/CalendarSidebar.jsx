...
const CalendarCard = ({
  ...
  const renderCalendarGrid = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // 1) Determine semantic status independent of "today"
      let status = '';
      let statusClass = ''; // background/text color based on status
      ...
      if (dayData?.isHoliday) {
        ...
        switch (dayData.attendance.status) {
          ...
          case 'leave':
            ...
            break;
          default:
            statusClass = isWeekend ? 'bg-gray-200 text-slate-700' : '';
            break;
        }
      } else if (isWeekend) {
        status = date.getDay() === 0 ? 'Sunday' : 'Saturday';
        statusClass = 'bg-gray-200 text-slate-500';
      }

      // 2) Today outline (always if today)
      const todayOutlineClass = isToday ? 'border-black border-2' : '';

      // 3) Fallback text color when no status class
      const fallbackText = isFutureDate
        ? 'text-slate-400'
    ...
    <div className="space-y-3">
      {tasks.map((task, index) => (
        <div key={index} className="flex items-center gap-4 group">
          <div
            className={`w-10 h-10 rounded-lg flex-shrink-0 flex items-center justify-center ${task.bgColor}`}
          >
            {React.cloneElement(task.icon, {
              className: `w-5 h-5 ${task.iconColor}`,
            })}
          </div>
...
const QuickActionsCard = ({ onApplyLeave }) => (
  <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
    ...
    <div className="space-y-3">
      <button
        className="w-full flex items-center cursor-pointer gap-3 text-left p-3 rounded-lg hover:bg-slate-100 transition-colors"
...
const DashboardRightPanel = () => {
  ...
  const calculateAttendancePercentage = (data) => {
    ...
    const workingDays = data.filter(
      (day) =>
        !day.isWeekend && !day.isHoliday && !day.isFutureDate && day.attendance,
    );
