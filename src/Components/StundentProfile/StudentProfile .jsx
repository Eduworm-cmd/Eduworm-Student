...
export default function StudentProfile() {
  ...
  const { loading, setLoading, attendanceData } = useStudentProfile(student);
      ...
      try {
        ...
        if (student) {
          const mappedStudentData = {
            ...
            admissionClass:
              student.class?.className || staticData.admissionClass,
            oldAdmissionNo: student.admissionNumber || "N/A",
            dateOfAdmission: student.dateOfJoining
            ...
            motherName: student.parents?.find(
              ...
              : "Not Available",
            fatherName: student.parents?.find(
            ...
            transferCertificate:
              student.documents?.transferCertificate || "N/A",
  ...
  const performanceMetrics = useMemo(() => {
    ...
    const { totalWorkingDays, presentDays, attendancePercentage } =
      attendanceData;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {activeView === "academics" && (
          <AcademicsView studentData={studentData} />
        )}
...
function RecentActivity() {
  const activities = [
    ...
    {
      type: "attendance",
      title: "Present - Chemistry Lab",
      time: "2 days ago",
      status: "present",
    },
    {
      type: "grade",
      title: "English Essay Graded: A-",
      time: "3 days ago",
      status: "graded",
    },
  ];
...
function AttendanceBreakdown({ attendanceData }) {
  const { totalWorkingDays, presentDays, absentDays,attendancePercentage } =
    attendanceData;
  ...
  const breakdown = [
    ...
    },
    {
      ...
      value: absentDays,
      color: "bg-red-500",
      percentage: ((absentDays / totalWorkingDays) * 100).toFixed(1),
    },
    {
      label: "Total WorkingDays",
...
function DataField({ label, value, icon: Icon, fullWidth = false }) {
  return (
    <div
      ...
      } flex items-center space-x-3 p-4 rounded-lg border border-slate-200 hover:border-slate-300 transition-colors`}
    >
