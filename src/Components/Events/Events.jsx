...
import QuillContent from "../QuillContent/QuillContent";
...
export const Events = () => {
  ...
  const stats = useMemo(() => {
    ...
    const today = filteredEvents.filter(
      (e) => parseInt(e.leftDay) === 0
    ).length;
    const week = filteredEvents.filter(
      (e) => parseInt(e.leftDay) > 0 && parseInt(e.leftDay) <= 7
    ).length;
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-600">
                Stay updated with what's happening in your branch
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {stats.map((s, idx) => (
            <motion.div
              ...
              className="flex gap-2 bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl p-3 shadow-sm"
            >
              <div
                className={`w-12 h-12 rounded-xl bg-gradient-to-br ${s.color} text-white flex items-center justify-center shadow-md mb-2`}
              >
                {s.icon}
              </div>
              <div className="flex flex-col">
                ...
                <div className="text-md text-slate-600">{s.label}</div>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredEvents?.map((event, index) => (
            ...
            >
              <span
                className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-semibold ${getDaysLeftColor(
                  event.leftDay
                )}`}
              >
              ...
              >
                ...
                <div className="p-6">
                      ...
                      <div className="flex items-center">
                        <div
                          className={`w-3 h-3 rounded-full mr-3 ${
                            getDaysLeftColor(selectedEvent.leftDay).split(
                              " "
                            )[0]
                          }`}
                        ></div>
                        <div>
                          ...
                          <p className="text-gray-600">
                            {getDaysLeftText(selectedEvent.leftDay)}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-gray-50 rounded-xl">
                    ...
                    <QuillContent htmlContent={selectedEvent?.notes} />
                  </div>
                </div>
                ...
                <div className="flex justify-end p-6 border-t border-gray-200">
                  <button
                    onClick={() => setSelectedEvent(null)}
                    className="bg-gray-700 text-white cursor-pointer px-6 py-2 rounded-lg hover:bg-gray-800 transition-colors"
                  >
                    Close
                  </button>
                </div>
