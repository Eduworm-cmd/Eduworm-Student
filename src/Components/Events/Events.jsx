import { Calendar, EyeIcon, FileText, Users, X, Search, Filter, CalendarDays, Sparkles } from 'lucide-react';
...
export const Events = () => {
    ...
    const getDaysLeftColor = (daysLeft) => {
        ...
        if (days <= 3) return 'bg-orange-100 text-orange-800 border border-orange-200';
    ...
    const filteredEvents = useMemo(() => {
        ...
        return list.filter(ev => {
          ...
          if (activeFilter === 'today') return matchSearch && days === 0;
          if (activeFilter === 'urgent') return matchSearch && days > 0 && days <= 3;
          if (activeFilter === 'week') return matchSearch && days > 3 && days <= 7;
          if (activeFilter === 'upcoming') return matchSearch && days > 7;
          return matchSearch;
        });
    ...
    const stats = useMemo(() => {
      ...
      const urgent = filteredEvents.filter(e => parseInt(e.leftDay) > 0 && parseInt(e.leftDay) <= 3).length;
      const week = filteredEvents.filter(e => parseInt(e.leftDay) > 3 && parseInt(e.leftDay) <= 7).length;
      return [
        { label: 'Total', value: total, color: 'from-blue-500 to-cyan-500' },
        { label: 'Today', value: today, color: 'from-green-500 to-emerald-500' },
        { label: 'Urgent', value: urgent, color: 'from-orange-500 to-red-500' },
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-blue-100/70 to-white">
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                {/* Search and Filter */}
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="flex flex-col lg:flex-row lg:items-center gap-4 mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['all', 'today', 'urgent', 'week', 'upcoming'].map(f => (
                      <button
                        key={f}
                        onClick={() => setActiveFilter(f)}
                        className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                          activeFilter === f ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg' : 'bg-white/70 text-slate-700 hover:bg-white/90'
                        }`}
                      >
