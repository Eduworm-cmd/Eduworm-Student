    ...
    >
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
        <div>
          ...
          <motion.p
            variants={itemVariants}
            className="mt-2 text-slate-600 max-w-lg"
          >
            Ready to learn something new today? Check your assignments and
            upcoming events.
          </motion.p>
        </div>
...
const Header = () => {
  ...
  const SchoolName =
    useSelector((state) => state.auth?.user?.branch) || "Modern School";
              ...
              <div className="p-4 pt-3">
                ...
                <div className="flex justify-between items-center mb-5">
                  ...
                  <motion.button
                    whileTap={{ scale: 0.9 }}
                    onClick={() => setShowMore(false)}
                    className="p-2 rounded-full text-slate-500 hover:bg-slate-200/70"
                  >
                    <X size={22} />
                  </motion.button>
                </div>
                ...
                >
                  {moreItems.map((item, idx) => (
                    ...
                    >
                      <div
                        className={`flex items-center justify-center h-16 w-16 rounded-2xl mb-2 transition-all duration-300 ${item.bgColor} shadow-sm`}
                      >
                        {React.cloneElement(item.icon, {
